...
function saveLikedPostsToSession(likedPosts) {
    var xhr = new XMLHttpRequest();
    xhr.open('POST', 'actions/save_liked_posts.php', true);
    xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
    xhr.onload = function () {
        if (xhr.status === 200) {
            console.log('Liked posts saved to session successfully');
        } else {
            console.error('Failed to save liked posts to session');
        }
    };
    var data = 'likedPosts=' + encodeURIComponent(JSON.stringify(likedPosts));
    xhr.send(data);
}
